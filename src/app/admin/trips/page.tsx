...
export default function AdminTrips() {
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div>
        <h2 className="text-2xl font-serif font-bold text-slate-800">Trip Management</h2>
                    ...
                    <tr key={trip.id} className={isActioning ? "opacity-50 pointer-events-none" : ""}>
                      <td className="px-5 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-slate-800">{trip.title || "Untitled Trip"}</span>
                          <span className="text-[var(--color-brand-green)] text-xs font-semibold mt-0.5 flex items-center gap-1">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2.5">
                          <img
                            ...
                            className="h-8 w-8 rounded-xl object-cover ring-1 ring-slate-150 flex-shrink-0"
                          />
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-slate-800 truncate">{trip.user?.name || "Unnamed"}</p>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5 text-xs text-slate-500">
                          ...
                          <span className="ml-1 text-[10px] text-slate-500 font-semibold bg-slate-50 border border-slate-200 rounded px-1">
                            {duration}d
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        {trip._count.itineraries > 0 ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 text-amber-700 bg-amber-50 px-2.5 py-1 rounded-full text-xs font-semibold border border-amber-200 hover:bg-amber-100 transition-all"
                          >
                            <FileText className="h-3 w-3" />
                            {trip._count.itineraries} Days (AI)
                          </button>
                        ) : (
                          <span className="text-slate-400 text-xs italic">No itinerary</span>
                        )}
                      </td>
                      <td className="px-5 py-4">
          ...
          <section className="w-full max-w-xl bg-white border border-slate-200 rounded-2xl p-6 shadow-2xl max-h-[85vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
            <div className="border-b border-slate-100 pb-4 mb-4 flex-shrink-0">
              ...
              <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                Destination: <strong className="text-[var(--color-brand-green)]">{selectedTrip.destination}</strong> | Creator: <strong className="text-slate-700">{selectedTrip.user?.name || selectedTrip.user?.email}</strong>
              </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-4 py-2">
              {loadingItineraries ? (
                <div className="flex h-32 items-center justify-center">
                  <div className="w-6 h-6 border-4 border-[var(--color-brand-green)] border-t-transparent rounded-full animate-spin" />
                </div>
                ...
                <div className="relative pl-6 border-l-2 border-slate-200 space-y-6">
                  {itineraries.map((day) => (
                    <div key={day.id} className="relative">
                      <span className="absolute -left-[31px] top-0.5 flex h-4 w-4 items-center justify-center rounded-full border-2 border-amber-500 bg-white">
                        <span className="h-1.5 w-1.5 rounded-full bg-amber-500" />
                      </span>
                      <h4 className="text-sm font-bold text-slate-800">Day {day.day}</h4>
                      <div className="mt-1.5 text-sm text-slate-600 leading-relaxed bg-slate-50 border border-slate-150 rounded-xl p-3.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <section className="w-full max-w-sm bg-white border border-slate-200 rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-slate-500 leading-relaxed mb-6">
              Permanently delete <strong className="text-slate-800">{deleteConfirmTrip.title || "Untitled Trip"}</strong> to <strong className="text-slate-800">{deleteConfirmTrip.destination}</strong>?
              This will remove all associated budgets, expenses, and itinerary data. <span className="text-red-600 font-semibold">This cannot be undone.</span>
            </p>
